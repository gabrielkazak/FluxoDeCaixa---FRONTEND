  ...
  const handleSubmitSendEmail = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFeedbackMessage(
          result.message || 'Erro ao enviar o e-mail de redefinição.'
        );
      }
    } catch (error) {
      console.error('Erro ao enviar o e-mail de redefinição:', error);
      setFeedbackMessage(
        'Erro ao enviar o e-mail de redefinição. Verifique sua conexão ou tente novamente mais tarde.'
      );
    }
  };
  ...
  const handleSubmitChangePassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFeedbackMessage(
          result.message ||
            'Erro ao redefinir senha. O token pode ser inválido ou expirado.'
        );
      }
    } catch (error) {
    ...
    <div className='corpo'>
      <div className='container rec-pass d-flex flex-column bg-light justify-content-center align-items-center rounded text-center'>
        ...
        {action === 'emailSent' && (
          <>
            <div className='action-text fs-5'>
              Um e-mail foi enviado para você com instruções para redefinir sua
              senha. Caso não apareça na sua caixa de entrada, verifique a pasta
              de spam.
              <br />
              <br />
              ...
              <span
                ...
                className='try-again-link text-primary'
              >
                {' '}
                clique aqui
              </span>{' '}
